...
layui.define(["laytpl", "laypage", "layer", "form", "util"],
  function(e) {
    var t = layui.$,
      ...
      z = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">', "{{# if(d.data.toolbar){ }}", '<div class="layui-table-tool">', '<div class="layui-table-tool-temp"></div>', '<div class="layui-table-tool-self"></div>', "</div>", "{{# } }}", '<div class="layui-table-box">', "{{# if(d.data.loading){ }}", '<div class="layui-table-init" style="background-color: #fff;">', ' <div id="loading_box">\n' +
      '      <div id="loading-center">\n' +
      '        <div id="loading-center-absolute">\n' +
      '          <div class="object-container" id="object_one"></div>\n' +
      '          <div class="object-container" id="object_two"></div>\n' +
      '          <div class="object-container" id="object_three"></div>\n' +
      '        </div>\n' +
      '      </div>\n' +
      '    </div>', "</div>", "{{# } }}", "{{# var left, right; }}", '<div class="layui-table-header">', _(), "</div>", '<div class="layui-table-body layui-table-main">', E, "</div>", "{{# if(left){ }}", '<div class="layui-table-fixed layui-table-fixed-l">', '<div class="layui-table-header">', _({
        fixed: !0
    ...
    F.prototype.config = {
      ...
      F.prototype.renderToolbar = function() {
        ...
        "object" == typeof a.defaultToolbar && layui.each(a.defaultToolbar,
          function(e, t) {
            var i = r[t];
            i && d.push('<div class="layui-inline" title="' + i.title + '" lay-event="' + i.layEvent + '"><i class="layui-icon ' + i.icon + '"></i>'+i.value+'</div>')
          }),
        ...
        if (i.startTime = (new Date).getTime(), a.url) {
          ...
          a.contentType && 0 == a.contentType.indexOf("application/json") && (d = JSON.stringify(d)),
            t.ajax({
              ...
              headers: a.headers || {},
              success: function(t) {
                var tips_paragraph = null;
                var statusCode = t.code;
                if(statusCode==117){
                  tips_paragraph='<i class="layui-icon mr-6">&#xe664;</i>您的登录信息已经失效，需要重新登录';
                  console.log('token失效')
                }else if(statusCode==118){
                  tips_paragraph='<i class="layui-icon mr-6">&#xe60c;</i>对不起，您无权限访问当前的页面数据'
                  console.log('无权限')
                }else if(statusCode==119){
                  tips_paragraph='<i class="layui-icon mr-6">&#xe664;</i>请求的参数可能存在异常'
                }else {
                  tips_paragraph='<i class="layui-icon mr-6">&#xe702;</i>'+t.message;
                }
                'function' == typeof a.parseData && (t = a.parseData(t) || t),

                  t[n.statusName] != n.statusCode ? (i.renderForm(), i.layMain.html('<div class="' + f + '">'+tips_paragraph+'</div>')) : (i.renderData(t, e, t[n.countName]), o(), a.time = (new Date).getTime() - i.startTime + " ms"),
                  i.setColsWidth(),
      ...
      F.prototype.loading = function(e) {
        var i = this,
          a = i.config;
        a.loading && (e ? (i.layInit && i.layInit.remove(), delete i.layInit, i.layBox.find(g).remove()) : (i.layInit = t(['<div class="layui-table-init">', '<i class="layui-icon layui-icon-loading-1 layui-icon"></i>', "</div>"].join("")), i.layBox.append(i.layInit)))
      },
            ...
            switch (layui.stope(e), R.trigger("table.tool.panel.remove"), l.close(a.tipsIndex), c) {
              ...
              case "LAYTABLE_EXPORT":
                r.ie ? l.tips("导出功能不支持 IE，请用 Chrome 等高级浏览器导出", this, {
                  ...
                  list: function() {
                    return ['<li data-type="csv">导出Csv 文件</li>', '<li data-type="xls">导出Excel 文件</li>'].join("")
                  } (),
                  done: function(e, i) {
                    i.on("click",
                      function() {
          ...
          b = function(e) {
           ...
           /* if (e) i.find(".layui-table-grid-down").remove();
            ...
            if (e && e == "hide"){
              ...
              ti = l.tips(a.text(), i, {tips:[1,'#3595CC'],time:0});
            }
          };
