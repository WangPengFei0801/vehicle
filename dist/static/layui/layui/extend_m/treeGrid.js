...
layui.config({
    ...
    if(iconOpen){
      ...
      if(isTreeNode){//父节点
        if((o[table.config.cols.iconOpen]||o[table.config.cols.iconClose])){
          ...
          temTreeHtml='<i class="layui-icon layui-tree-'+o[options.idField]+' layui-tree-'+ (o[table.config.cols.isOpen] ? "branch" : "leaf") +'" '+iconOpen+'>'+(o[table.config.cols.isOpen]?'&#xe663;':that.config.branch[0])+'</i>';
        }
      }else{//叶子节点
        if(o[table.config.cols.icon]){
          temTreeHtml=temf();
        }else{
          temTreeHtml+='<i class="layui-icon mr-4 layui-tree-'+o[options.idField]+'layui-tree-leaf"  '+iconOpen+'>&#xe705;</i>';
        }
      }
      if(isHtml){
        return temTreeHtml;
  ...
  Class.prototype.pullData = function(curr, loadIndex){
    ...
    if(options.url){ //Ajax请求
      var params = {};
      var token = JSON.parse(window.localStorage.getItem('token')).data;
