layui.define(["form"], function(exports) {
  ...
  tree.prototype.template = function(data) {
    ...
    o.each(data, function(index, item) {
      var hide_class = 'class="' + (item.pid == 0 || item.pid == t.cache(item.pid) || t.c.is_open ? "show" : "hide") + '"',
        tr = '<tr state="'+(item.pid==0?'':'state-controls')+'" data-id="' + item.id + '" data-pid="' + item.pid + '" ' + hide_class + ">" +
          (t.c.is_checkbox ? '<td><div><input type="checkbox" class="select-option" lay-skin="primary" lay-filter="lay-t"></div></td>' : "");
  ...
  tree.prototype.render = function(e) {
    ...
    form.render('checkbox').on('checkbox(lay-t)', function(data) {
      ...
      select_box.innerHTML = JSON.stringify(data);
      console.log(data)
  ...
  tree.prototype.hide = function(id) {
    var t = this;
    o(t.c.elem).find("tr[data-pid=" + id + "]").each(function() {
      o(this).addClass('hide').removeClass('show');
  ...
  tree.prototype.show = function(id) {
    var t = this;
    o(t.c.elem).find("tr[data-pid=" + id + "]").each(function() {
      o(this).removeClass('hide').addClass('show');
